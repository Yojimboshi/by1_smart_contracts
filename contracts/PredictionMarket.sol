...
contract PredictionMarket is EIP712, Ownable, ReentrancyGuard, Pausable {
  ...
  bytes32 public constant SETTLEMENT_TYPEHASH =
    keccak256(
      "Settlement(uint256 roundId,uint256 closePrice,uint8 outcome,uint256 chainId,address contractAddress,uint256 settledAt)"
    );
  ...
  event RoundSettled(string indexed roundId, uint256 closePrice, Outcome outcome);
  ...
  function createRound(
    ...
    require(bytes(rounds[roundId].roundId).length == 0, "Round already exists");
  ...
  function claimWinnings(string memory roundId) external nonReentrant {
    ...
    if (round.outcome == Outcome.TIE) {
      ...
      (round.outcome == Outcome.UP && bet.isUp) || (round.outcome == Outcome.DOWN && !bet.isUp)
    ) {
      // Win: 2x bet amount
      payout = bet.amount * 2;
    } else {
   ...
   */
  function getUserBet(string memory roundId, address user) external view returns (Bet memory) {
   ...
   */
  function getRound(string memory roundId) external view returns (Round memory) {
   ...
   */
  function getBettors(string memory roundId) external view returns (address[] memory) {
